...
下面的交互把“全量、裁剪、压缩、按需读取”放进同一个窗口预算中比较。切换策略会改变请求组成；调整窗口会看到同一请求是可发出、将触发压缩，还是已经超出窗口。策略会变，但模型请求始终由三个部分组成。

<ContextComposer />

交互中的 token 数值只用来比较策略，不是固定源码的实测数据。压缩判定使用 Coding Agent 默认 `reserveTokens = 16384`。可以确认的源码事实是组装顺序、字段边界和 compaction 规则。
